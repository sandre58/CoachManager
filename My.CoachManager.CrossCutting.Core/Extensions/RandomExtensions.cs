...
namespace My.CoachManager.CrossCutting.Core.Extensions
{
    ...
    public static class RandomExtensions
    {
            ...
            IDictionary<TKey, TValue> dictionary)
        {
            ...
            var key = dictionary.Keys.ToList()[random.Next(0, dictionary.Count)];

            return new KeyValuePair<TKey, TValue>(key, dictionary[key]);
        }

        #endregion ChooseItem
            ...
            bool allowDuplicates)
        {
            ...
            if (count > buffer.Count)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(count),
                    $@"Count is more than the size of the collection. Count:<{count}>");
            }
        ...
        public static string NextString(this Random random, int length, string characterRange)
        {
            ...
            for (var i = 0; stringBuilder.Length < length; i++)
            {
                var randomNumber = random.Next(0, characterRange.Length);
        ...
        #endregion NextAlphanumericString
