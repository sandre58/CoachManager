...
namespace My.CoachManager.Presentation.Prism.Tests
{
    ...
    ///  <p><see>
    ///          <cref>OrderedBag&amp;lt;T&amp;gt;</cref>
    ///      </see>
    ///      is similar, but uses comparison instead of hashing, maintain
    ...
    public class Bag<T> : CollectionBase<T>, ICloneable
    {
        ...
        private readonly IEqualityComparer<KeyValuePair<T, int>> _equalityComparer;
        ...
        private Hash<KeyValuePair<T, int>> _hash;
        ...
        public sealed override void Add(T item)
        {
            ...
            if (!_hash.Insert(pair, false, out existing))
            {
                ...
                _hash.Insert(newPair, true, out pair);
            }
            ++_count;
        }
        ...
        public Bag<T> Intersection(Bag<T> otherBag)
        {
            ...
            // Enumerate each of the items in the smaller bag. Add items that need to be
            // added to the intersection.
            result = new Bag<T>(_keyEqualityComparer);
            foreach (T item in smaller.DistinctItems())
            {
                copiesInLarger = larger.NumberOfCopies(item);
                copiesInSmaller = smaller.NumberOfCopies(item);
