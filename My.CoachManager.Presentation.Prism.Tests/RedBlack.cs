...
namespace My.CoachManager.Presentation.Prism.Tests
{
    ...
    {
        private IComparer<T> _comparer;         // interface for comparing elements, only Compare is used.
        private Node _root;                 // The root of the tree. Can be null when tree is empty.
        private int _count;						// The count of elements in the tree.

        private int _changeStamp;        // An integer that is changed every time the tree structurally changes.
                                         // Used so that enumerations throw an exception if the tree is changed
                                         // during enumeration.

        private Node[] _stack;               // A stack of nodes. This is cached locally to avoid constant re-allocated it.
        ...
        private Node[] GetNodeStack()
        {
            ...
            if (_count < 0x400)
                maxDepth = 21;
            else if (_count < 0x10000)
                maxDepth = 41;
            else
                maxDepth = 65;

            if (_stack == null || _stack.Length < maxDepth)
                _stack = new Node[maxDepth];
        ...
        private void CheckEnumerationStamp(int startStamp)
        {
            ...
            {
                throw new InvalidOperationException("ChangeDuringEnumeration");
            }
        }
        ...
        public bool Find(T key, bool findFirst, bool replace, out T item)
        {
            ...
            while (current != null)
            {
                int compare = _comparer.Compare(key, current.Item);
        ...
        public RangeTester BoundedRangeTester(bool useFirst, T first, bool useLast, T last)
        {
            ...
            {
                if (useFirst && _comparer.Compare(first, item) > 0)
                    return -1;     // item is before first.
                else if (useLast && _comparer.Compare(last, item) <= 0)
        ...
        public RangeTester DoubleBoundedRangeTester(T first, bool firstInclusive, T last, bool lastInclusive)
        {
            return delegate (T item)
            {
                if (firstInclusive)
                {
                    if (_comparer.Compare(first, item) > 0)
                        return -1;     // item is before first.
                }
                else
                {
                ...
                if (lastInclusive)
                {
                    if (_comparer.Compare(last, item) < 0)
        ...
        public RangeTester EqualRangeTester(T equalTo)
        {
            return delegate (T item)
            {
                return _comparer.Compare(item, equalTo);
            };
        }
        ...
        {
            return EnumerateRangeInOrder(rangeTester, _root);
        }
        ...
        {
            return EnumerateRangeInReversedOrder(rangeTester, _root);
        }
        ...
        {
            return CountRangeUnderNode(rangeTester, _root, false, false);
        }
        ...
        {
            Debug.Assert(_comparer != null, "Comparer should not be null");

            if (_root == null)
            {
                Debug.Assert(0 == _count, "Count in empty tree should be 0.");
            }
            else
            {
        ...
        private int ValidateSubTree(Node node, out int blackHeight)
        {
            ...
            if (node.Left != null)
                Debug.Assert(_comparer.Compare(node.Left.Item, node.Item) <= 0, "Left child is not less than or equal to node");
            if (node.Right != null)
                Debug.Assert(_comparer.Compare(node.Right.Item, node.Item) >= 0, "Right child is not greater than or equal to node");
