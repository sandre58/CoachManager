...
namespace My.CoachManager.Presentation.Prism.Tests
{
    ...
    internal class Hash<T> : IEnumerable<T>, ISerializable, IDeserializationCallback
    {
        ...
        private struct Slot
        {
            private uint _hashCollision;   // Lower 31 bits: the hash value. Top bit: the collision bit.
            ...
            public int HashValue
            {
                get
                {
                    return (int)(_hashCollision & 0x7FFFFFFF);
                }
                set
                {
                    Debug.Assert((value & 0x80000000) == 0);  // make sure sign bit isn't set.
                    _hashCollision = (uint)value | (_hashCollision & 0x80000000);
                }
            }
            ...
            public bool Collision
            {
                get
                {
                    return (_hashCollision & 0x80000000) != 0;
                }
                set
                {
                    if (value)
                        _hashCollision |= 0x80000000;
                    else
                        _hashCollision &= 0x7FFFFFFF;
                }
            }
        }
        ...
        private int GetFullHash(T item)
        {
            ...
            hash = (uint)Util.GetHashCode(item, _equalityComparer);
        ...
        public float LoadFactor
        {
            ...
            set
            {
                ...
                if (value < 0.25 || value > 0.95)
                    throw new ArgumentOutOfRangeException("value", value, @"InvalidLoadFactor");
        ...
        {
            Console.WriteLine(@"loadFactor={0}  thresholdGrow={1}  thresholdShrink={2}", _count, _usedSlots, _totalSlots);
            Console.WriteLine(@"loadFactor={0}  thresholdGrow={1}  thresholdShrink={2}", _loadFactor, _thresholdGrow, _thresholdShrink);
            Console.WriteLine(@"hashMask={0:X}  secondaryShift={1}", _hashMask, _secondaryShift);
            Console.WriteLine();
        }
        ...
        internal void Print()
        {
            ...
            for (int i = 0; i < _totalSlots; ++i)
                Console.WriteLine(@"Slot {0,4:X}: {1} {2,8:X} {3}", i, _table[i].Collision ? "C" : " ",
