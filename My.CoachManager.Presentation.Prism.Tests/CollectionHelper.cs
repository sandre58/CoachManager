...
namespace My.CoachManager.Presentation.Prism.Tests
{
    ...
    /// <summary>
    /// CollectionHelper contains a number of static methods that implement
    /// CollectionHelper that work on collections. Most of the methods deal with
    ...
    public static class CollectionHelper
    {
        ...
        private class ListRange<T> : ListBase<T>, ICollection<T>
        {
            ...
            public override int Count
            {
                get
                {
                    return Math.Min(_count, _wrappedList.Count - _start);
                }
            }
            ...
            public override bool Remove(T item)
            {
                if (_wrappedList.IsReadOnly)
                    throw new NotSupportedException("CannotModifyCollection");
            ...
            bool ICollection<T>.IsReadOnly
            {
                get
                {
                    return _wrappedList.IsReadOnly;
                }
            }
        }
        ...
        /// <code>CollectionHelper.ReverseInPlace(CollectionHelper.Range(list, 3, 6))</code>
        ...
        private class ArrayRange<T> : ListBase<T>
        {
            ...
            public override void Clear()
            {
                Array.Copy(_wrappedArray, _start + _count, _wrappedArray, _start, _wrappedArray.Length - (_start + _count));
                FillRange(_wrappedArray, _wrappedArray.Length - _count, _count, default(T));
                _count = 0;
            }
        ...
        /// <code>CollectionHelper.ReverseInPlace(CollectionHelper.Range(array, 3, 6))</code>
        /// will reverse the 6 items beginning at index 3.</remarks>
        ...
        private class ReadOnlyDictionary<TKey, TValue> : IDictionary<TKey, TValue>
        {
            ...
            {
                get { return ReadOnly(_wrappedDictionary.Keys); }
            }

            public ICollection<TValue> Values
            {
                get { return ReadOnly(_wrappedDictionary.Values); }
            }
        ...
        /// are of type <paramref>
        ///         <name>T</name>
        ///     </paramref>
        ///     or a type derived from it. This method is useful
        ...
        /// items of type <paramref>
        ///         <name>T</name>
        ///     </paramref>
        ///     or a type derived from it. </param>
        ...
        private class TypedCollection<T> : ICollection<T>
        {
            ...
            public bool Contains(T item)
            {
                ...
                foreach (object obj in _wrappedCollection)
                {
        ...
        ///     </paramref>
        ///     or a type derived from it. </param>
        /// <returns>A generic IList&lt;T&gt; wrapper around <paramref>
        ///         <name>untypedlist</name>
        ///     </paramref>
        ///     .
        /// If <paramref>
        ///         <name>untypedlist</name>
        ///     </paramref>
        ///     is null, then null is returned.</returns>
        ...
        private class UntypedCollection<T> : ICollection
        {
            ...
            public void CopyTo(Array array, int index)
            {
                ...
                if (index < 0)
                    throw new ArgumentOutOfRangeException("index", index, @"ArgMustNotBeNegative");
                if (index >= array.Length || count > array.Length - index)
                    throw new ArgumentException(@"index", $"array");
        ...
        private class UntypedList<T> : IList
        {
            ...
            private T ConvertToItemType(object value)
            {
                try
                {
                    return (T)value;
            ...
            public int Add(object value)
            {
                ...
                _wrappedList.Add(ConvertToItemType(value));
            ...
            { _wrappedList.Insert(index, ConvertToItemType(value)); }
            ...
            {
                get { return _wrappedList[index]; }
                set { _wrappedList[index] = ConvertToItemType(value); }
            }
        ...
        /// <para>Use this method when you need to pass an array to an CollectionHelper that takes an
        /// IList&lt;T&gt; and that tries to modify items in the list. CollectionHelper in this class generally do not
        ...
        /// <param name="itemFind">The value to find and replace within <paramref>
        ...
        public static void ReplaceInPlace<T>(IList<T> list, T itemFind, T replaceWith, IEqualityComparer<T> equalityComparer)
        {
            ...
            if (list.IsReadOnly)
                throw new ArgumentException(@"ListIsReadOnly", $"list");
        ...
        public static IEnumerable<int> IndicesOfMany<T>(IList<T> list, IEnumerable<T> itemsToLookFor, BinaryPredicate<T> predicate)
        {
            ...
            // Scan the list for the items.
            int index = 0;
            var toLookFor = itemsToLookFor as T[] ?? itemsToLookFor.ToArray();
            foreach (T x in list)
            {
                foreach (T y in toLookFor)
                {
                    if (predicate(x, y))
                    {
                        yield return index;
        ...
        private static volatile Random _myRandomGenerator;
        ...
        {
            if (_myRandomGenerator == null)
            {
                lock (typeof(CollectionHelper))
                {
                    if (_myRandomGenerator == null)
                        _myRandomGenerator = new Random();
                }
            }

            return _myRandomGenerator;
        }
        ...
        /// </summary>
        /// <remarks><para>The Quicksort CollectionHelper is used to sort the items. In virtually all cases,
                    ...
                    else
                    {
                        ...
                        for (;;)
                        {
                            do
                            {
                                ++i;
                                itemI = list[i];
                            } while (comparer.Compare(itemI, partition) < 0);

                            do
                            {
                                --j;
                                itemJ = list[j];
                            } while (comparer.Compare(itemJ, partition) > 0);

                            if (j < i)
                                break;

                            list[i] = itemJ; list[j] = itemI; // swap items to continue the partition.
                        }
        ...
        public static void StableSortInPlace<T>(IList<T> list, IComparer<T> comparer)
        {
            ...
            // The stable sort CollectionHelper also uses QuickSort. An additional array of indices (order) is
            ...
            for (;;)
            {
                ...
                else if (l < r)
                {
                    ...
                    else
                    {
                        ...
                        list[r] = partition = e2; order[r] = orderPartition = o2;
                        ...
                        for (;;)
                        {
                            ...
                            {
                                ++i;
                                itemI = list[i]; orderI = order[i];
                            } while ((c = comparer.Compare(itemI, partition)) < 0 || c == 0 && orderI < orderPartition);

                            do
                            {
                                --j;
                                itemJ = list[j]; orderJ = order[j];
                            } while ((c = comparer.Compare(itemJ, partition)) > 0 || c == 0 && orderJ > orderPartition);

                            if (j < i)
                                break;

                            list[i] = itemJ; list[j] = itemI; // swap items to continue the partition.
                            order[i] = orderJ; order[j] = orderI;
                        }

                        // Move the partition value into place.
                        list[r] = itemI; order[r] = orderI;
                        list[i] = partition; order[i] = orderPartition;
                        ++i;
        ...
        private class LexicographicalComparerClass<T> : IComparer<IEnumerable<T>>
        {
            ...
            {
                return LexicographicalCompare(x, y, _itemComparer);
            }
            ...
            public override bool Equals(object obj)
            {
                if (obj is LexicographicalComparerClass<T>)
                    return _itemComparer.Equals(((LexicographicalComparerClass<T>)obj)._itemComparer);
                return false;
            }

            public override int GetHashCode()
            {
                return _itemComparer.GetHashCode();
            }
        }
        ...
        /// for collections or CollectionHelper that use sequences of T as an item type. The Lexicographical
        ...
        /// for collections or CollectionHelper that use sequences of T as an item type. The Lexicographics
        /// ordered of sequences is for comparison.
        /// </summary>
        ...
        private class ReverseComparerClass<T> : IComparer<T>
        {
            ...
            public override bool Equals(object obj)
            {
                if (obj is ReverseComparerClass<T>)
                    return _comparer.Equals(((ReverseComparerClass<T>)obj)._comparer);
                return false;
            }
        ...
        private class CollectionEqualityComparer<T> : IEqualityComparer<IEnumerable<T>>
        {
            ...
            {
                return EqualCollections(x, y, _equalityComparer);
            }
            ...
            public int GetHashCode(IEnumerable<T> obj)
            {
                ...
                foreach (T t in obj)
                {
                    int itemHash = Util.GetHashCode(t, _equalityComparer);
                    hash += itemHash;
                    hash = (hash << 9) | (hash >> 23);
                }
        ...
        /// in CollectionHelper.EqualCollections, but encapsulated in an IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation.
        /// </summary>
        /// <example>
        /// The following code creates a Dictionary where the keys are a collection of strings.
        /// <code>
        ///     Dictionary&lt;IEnumerable&lt;string&gt;, int&gt; =
        ///         new Dictionary&lt;IEnumerable&lt;string&gt;, int&gt;(CollectionHelper.GetCollectionEqualityComparer&lt;string&gt;());
        /// </code>
        /// </example>
        /// <returns>IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation suitable for
        /// comparing collections of T for equality.</returns>
        /// <seealso>
        ///     <cref>CollectionHelper.EqualCollections</cref>
        /// </seealso>
        ...
        /// of T's are equal if they have the same number of items, and corresponding
        /// items are equal, considered in order. This is the same notion of equality as
        /// in CollectionHelper.EqualCollections, but encapsulated in an IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation.</para>
        ...
        /// <code>
        ///     Dictionary&lt;IEnumerable&lt;string&gt;, int&gt; =
        ///         new Dictionary&lt;IEnumerable&lt;string&gt;, int&gt;(CollectionHelper.GetCollectionEqualityComparer&lt;string&gt;(StringComparer.CurrentCultureIgnoreCase));
        /// </code>
        /// </example>
        ...
        private class SetEqualityComparer<T> : IEqualityComparer<IEnumerable<T>>
        {
            ...
            public bool Equals(IEnumerable<T> x, IEnumerable<T> y)
            {
                return EqualSets(x, y, _equalityComparer);
            }
        ...
        /// in CollectionHelper.EqualSets, but encapsulated in an IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation.</para>
        /// <para>An IEqualityComparer&lt;T&gt; is used to determine if individual T's are equal</para>
        /// </summary>
        /// <example>
        ...
        /// <code>
        ///     Dictionary&lt;IEnumerable&lt;string&gt;, int&gt; =
        ///         new Dictionary&lt;IEnumerable&lt;string&gt;, int&gt;(CollectionHelper.GetSetEqualityComparer&lt;string&gt;(StringComparer.CurrentCultureIgnoreCase));
        /// </code>
        /// </example>
        /// <returns>IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation suitable for
        /// comparing collections of T for equality, without regard to order.</returns>
        /// <seealso>
        ///     <cref>CollectionHelper.EqualSets</cref>
        /// </seealso>
        ...
        /// of T's are equal if they have the same number of items, and corresponding
        /// items are equal, without regard to order. This is the same notion of equality as
        /// in CollectionHelper.EqualSets, but encapsulated in an IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation.
        /// </summary>
        /// <example>
        /// The following code creates a Dictionary where the keys are a set of strings, without regard to order
        /// <code>
        ///     Dictionary&lt;IEnumerable&lt;string&gt;, int&gt; =
        ///         new Dictionary&lt;IEnumerable&lt;string&gt;, int&gt;(CollectionHelper.GetSetEqualityComparer&lt;string&gt;());
        /// </code>
        /// </example>
        /// <param name="equalityComparer">An IEqualityComparer&lt;T&gt; implementation used to compare individual T's.</param>
        /// <returns>IEqualityComparer&lt;IEnumerable&lt;T&gt;&gt; implementation suitable for
        ...
        public static ICollection<T> RemoveWhere<T>(ICollection<T> collection, Predicate<T> predicate)
        {
            ...
            if (collection.IsReadOnly)
                throw new ArgumentException(@"ListIsReadOnly", "collection");
        ...
        /// <code>
        /// List&lt;double&gt; list = new List&lt;double&gt;(CollectionHelper.NCopiesOf(1000, 1.0));
        /// </code></example>
        ...
        {
            if (n < 0)
                throw new ArgumentOutOfRangeException("n", n, @"ArgMustNotBeNegative");
            ...
            using (IEnumerator<T> sourceEnum = source.GetEnumerator())
            {
                ...
                while (sourceEnum.MoveNext())
                {
                    if (destIndex >= dest.Length)
                        throw new ArgumentException(@"ArrayTooSmall", $"dest");
        ...
        public static void Copy<T>(IEnumerable<T> source, IList<T> dest, int destIndex, int count)
        {
            ...
            if (dest == null)
                throw new ArgumentNullException("dest");
            if (dest.IsReadOnly)
                throw new ArgumentException(@"ListIsReadOnly", "dest");
