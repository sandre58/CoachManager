    ...
    public class OrderedSet<T> : CollectionBase<T>, ICollection<T>, ICloneable
    {
        ...
        public OrderedSet<T> CloneContents()
        {
            ...
            if (!Util.IsCloneableType(typeof(T), out itemIsValueType))
                throw new InvalidOperationException("TypeNotCloneable");
        ...
        private class ListView : ReadOnlyListBase<T>
        {
            ...
            public override int Count
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        return mySet._tree.CountRange(rangeTester);
                    }
                }
            }
            ...
            public override T this[int index]
            {
                get
                {
                    ...
                    {
                        T dummy;
                        int firstIndex = mySet._tree.FirstItemInRange(rangeTester, out dummy);
                        int lastIndex = mySet._tree.LastItemInRange(rangeTester, out dummy);
            ...
            public override int IndexOf(T item)
            {
                ...
                else
                {
                    ...
                    if (reversed)
                    {
                        int indexInSet = mySet._tree.FindIndex(item, false);
                        if (indexInSet < 0)
                            return -1;
                        int indexOfEnd = mySet._tree.LastItemInRange(rangeTester, out dummy);
                        return indexOfEnd - indexInSet;
                    }
                    else
                    {
                        int indexInSet = mySet._tree.FindIndex(item, true);
                        if (indexInSet < 0)
                            return -1;
                        int indexOfStart = mySet._tree.FirstItemInRange(rangeTester, out dummy);
        ...
        public class View : CollectionBase<T>, ICollection<T>
        {
            ...
            {
                if (reversed)
                    return mySet._tree.EnumerateRangeReversed(rangeTester).GetEnumerator();
                else
                    return mySet._tree.EnumerateRange(rangeTester).GetEnumerator();
            }
            ...
            public sealed override void Clear()
            {
                ...
                {
                    mySet._tree.DeleteRange(rangeTester);
                }
            }
            ...
            {
                if (!ItemInView(item))
                    throw new ArgumentException(@"OutOfViewRange", "item");
                else
            ...
            public int IndexOf(T item)
            {
                if (entireTree)
                {
                    ...
                    {
                        return mySet._tree.FindIndex(item, true);
                    }
                }
                else
                {
            ...
            public T GetFirst()
            {
                ...
                if (reversed)
                    found = mySet._tree.LastItemInRange(rangeTester, out item);
                else
                    found = mySet._tree.FirstItemInRange(rangeTester, out item);
