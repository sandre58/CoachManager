...
{
    public class SchedulerBlackoutDatesCollection : ObservableCollection<SchedulerDateRange>
    {
        #region Data
        ...
        public SchedulerBlackoutDatesCollection(Scheduler owner)
        {
            _owner = owner;
        ...
        {
            Add(new SchedulerDateRange(DateTime.MinValue, DateTime.Today.AddDays(-1)));
        }
        ...
        public bool ContainsAny(SchedulerDateRange range)
        {
            foreach (SchedulerDateRange item in this)
            {
        ...
        protected override void ClearItems()
        {
            ...
            foreach (SchedulerDateRange item in Items)
            {
                UnRegisterItem(item);
            }
        ...
        protected override void InsertItem(int index, SchedulerDateRange item)
        {
            if (!IsValidThread())
            {
                throw new NotSupportedException();
        ...
        /// <param name="index"></param>
        /// <param name="item"></param>
        protected override void SetItem(int index, SchedulerDateRange item)
        {
            if (!IsValidThread())
            {
                throw new NotSupportedException();
            ...
            if (IsValid(item))
            {
                SchedulerDateRange oldItem = null;
                if (index >= 0 && index < Count)
                {
                    oldItem = Items[index];
                }
        ...
        /// </summary>
        /// <param name="item"></param>
        private void RegisterItem(SchedulerDateRange item)
        {
            if (item != null)
            {
        ...
        /// </summary>
        private void UnRegisterItem(SchedulerDateRange item)
        {
            if (item != null)
            {
                item.Changing -= Item_Changing;
        ...
        /// <param name="e"></param>
        private void Item_Changing(object sender, SchedulerDateRangeChangingEventArgs e)
        {
        ...
        private bool IsValid(SchedulerDateRange item)
        {
        ...
        {
            foreach (DateTime child in _owner.SelectedDatesInternal)
            {
                DateTime? day = child;
        ...
        /// <param name="date"></param>
        /// <returns></returns>
        private SchedulerDateRange GetContainingDateRange(DateTime date)
        {
